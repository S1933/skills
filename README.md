...
`./install.sh` is idempotent: every run wipes the previous install (respecting `.gitignore`), regenerates `skills-lock.json`, and reinstalls the full registry from scratch. There are no other flags by design — keep the surface small.
...
`skills-lock.json` is gitignored — it's a build artifact regenerated by `./install.sh` and contains the SHA256 of each installed SKILL.md for drift detection.
...
├── skills-lock.json       # gitignored — regenerated by ./install.sh
├── .gitignore             # excludes /arsenal/, /cdsv2/, etc. + /.agents/, /skills-lock.json
└── .github/workflows/
...
`/.agents/` and `/skills-lock.json` are `npx skills add` artifacts — regenerated by `./install.sh`, never versioned.
